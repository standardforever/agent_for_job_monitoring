...
class CareerCategoryNodeProcessor:
    ...
    def process(self, task: dict[str, Any], worker_name: str, celery_task_id: str) -> dict[str, Any]:
        slot = self._claim_slot(task, worker_name, celery_task_id)
        tokens = self._set_client_openai_config(task)
        try:
    ...
    def _claim_slot(self, task: dict[str, Any], worker_name: str, celery_task_id: str) -> dict[str, Any]:
    ...
    def _set_client_openai_config(self, task: dict[str, Any]):
        client = self._load_client_config(task)
        return set_openai_runtime_config(api_key=client.get("api_key"), model=client.get("model"))

    def _load_client_config(self, task: dict[str, Any]) -> dict[str, Any]:
        client_name = str(task.get("client_name") or "").strip()
